...
def get_mask_data(annFile, mask_to_save):
    ...
    for imageinfo in image_info_list:
        ...
        # 保存
        npy_name = mask_to_save + "/SegmentationClassPNG/" + imageinfo['file_name'][:-4] + '.png'
        png_name = mask_to_save + "SegmentationClass/" +  imageinfo['file_name'][:-4] + '.png'
        np.save(npy_name.split(".png")[0], mask_image)
        lbl_pil = Image.fromarray(mask_image.astype(np.uint8), mode="P")
        colormap = imgviz.label_colormap()
        lbl_pil.putpalette(colormap.flatten())
        lbl_pil.save(png_name)

        print("save mask: ", png_name)


if __name__ == '__main__':
    # 用来保存最后生成的mask图像目录
    mask_to_save = savepath + 'mask_images/'  # 单通道mask图存储路径
    # 生成路径
    mkr(mask_to_save)
    mkr(mask_to_save + "/SegmentationClass/")
    mkr(mask_to_save + "/SegmentationClassPNG/")
    # 获取要处理的json文件路径
